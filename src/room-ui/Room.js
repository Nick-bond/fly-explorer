import { makeBox, hex, solid } from './geometry.js';
...
//
// Also the spatial authority — actors and the camera ask it where they
// are allowed to be via `bounds`, `clampPoint` and `collideSphere`.
export class Room {
  constructor(renderer, { width = 6, height = 3, depth = 6 } = {}) {
    this.width = width;
    this.height = height;
    this.depth = depth;

    // Neutral shades only — the fly must identify objects by geometry,
    // not color. Brightness differs so visual boundaries stay obvious.
  ...
  // Clamp a point [x, y, z] to stay inside the room shell, `margin`
  // meters away from every surface. Mutates and returns the point.
  // (Walls only — use collideSphere to also respect the boxes.)
  clampPoint(p, margin = 0.25) {
    const b = this.bounds;
  ...
  // Collide a sphere (center p [x,y,z], radius r) against the room shell
  // and every obstacle box. Pushes p out of penetration (mutates it) and
  // returns an array of contact normals (unit vectors pointing away from
  // the surface). Empty array = no contact.
  collideSphere(p, r) {
    const hits = [];
    const b = this.bounds;
